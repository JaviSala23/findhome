...
class Propietario:
	...
	def agregar(self,con1):
		cone=con1.conectar()
		cur=cone.cursor()
		query="INSERT INTO propietario(nombre,direccion,localidad,telefono,celular,email) VALUES (%s,%s,%s,%s,%s,%s)"
	...
	def update(self,con1,filtro):
		cone=con1.conectar()
		cur=cone.cursor()
		query="UPDATE propietario SET nombre=%s, direccion=%s, localidad=%s, telefono=%s, celular=%s, email=%s WHERE nombre=%s "
	...
	def eliminar(self,con1,filtro):
		cone=con1.conectar()
		cur=cone.cursor()
		query="DELETE FROM propietario WHERE nombre=%s "
		valores=filtro
	...
	def consultar(self,con1):
		cone=con1.conectar()
		cur=cone.cursor()
		query="SELECT * FROM propietario WHERE nombre=%s ORDER BY nombre"
		registro=con1.consultar(query,cur,self.nombre)
		return registro
	def consultartodos(self,con1):
		cone=con1.conectar()
		cur=cone.cursor()
		query="SELECT * FROM propietario ORDER BY nombre"
